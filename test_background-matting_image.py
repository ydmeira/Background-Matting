...
for i in range(0,len(test_imgs)):
	...
	try:
		...
		cv2.imwrite(result_path+'/'+filename.replace('_img','_out'), alpha_out0) # alpha mask to composite (H, W)
		cv2.imwrite(result_path+'/'+filename.replace('_img','_fg'), cv2.cvtColor(fg_out0,cv2.COLOR_BGR2RGB)) # cropped foreground image (H, W, 3)
		...
		comp_im_tr2=composite4(bgr_img,back_img20,alpha_temp)
		cv2.imwrite(result_path+'/'+filename.replace('_img','_out'), alpha_temp) # alpha mask to composite (H, W)
		cv2.imwrite(result_path+'/'+filename.replace('_img','_fg'), cv2.cvtColor(bgr_img,cv2.COLOR_BGR2RGB)) # cropped foreground image (H, W, 3)
		cv2.imwrite(result_path+'/'+filename.replace('_img','_compose'), cv2.cvtColor(comp_im_tr1,cv2.COLOR_BGR2RGB)) # result compose (H, W, 3)
		cv2.imwrite(result_path+'/'+filename.replace('_img','_matte').format(i), cv2.cvtColor(comp_im_tr2,cv2.COLOR_BGR2RGB)) # result matte (H, W, 3)
		traceback.print_exc()
